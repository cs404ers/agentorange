...
public class AgentOrange extends Agent{
	...
	private HashMap agentpositions;		//the average positions of competing agents
	...
	/**
	 * List of all bid bundles made in the past by this agent	 
	 */
	protected Queue<BidBundle> bidBundles;
	
	private double bankBalance;			//the amount of cash that the agent curretly holds
	...
	**/
	...
	private double getPenultimateBid(Query q) {
		...
		if (temp.removeLast() == null) 
			return -1;
		
		//return the last element, we do care about this
		BidBundle bb =(BidBundle)((LinkedList)temp).removeLast();
		if (bb == null) {
			return -1;
		}
